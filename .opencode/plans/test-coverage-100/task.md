...
- [ ] Test `buildCollectionTree` with flat list → returns nested tree sorted by orderValue then name
- [ ] Test `buildCollectionTree` filters out archived collections
- [ ] Test `buildCollectionTree` filters out inbox collection
- [ ] Test `setCollections` replaces state
- [ ] Test `addCollection` appends to array
- [ ] Test `updateCollection` patches by id
- [ ] Test `removeCollection` removes by id

### Task 1.2 — app/stores/__tests__/taskStore.test.ts
- [ ] Test `setTasks` replaces state
- [ ] Test `addTask` appends
- [ ] Test `updateTask` patches by id
- [ ] Test `removeTask` removes by id

### Task 1.3 — app/api/__tests__/queryClient.test.ts
- [ ] Test `staleTime` equals 60000
- [ ] Test `retry` equals 1

### Task 1.4 — app/utils/__tests__/fontLoader.test.ts
- [ ] Test `ensureFontLoaded` creates a `<link>` element with correct href
- [ ] Test `ensureFontLoaded` resolves on load event
- [ ] Test `ensureFontLoaded` rejects on error event
- [ ] Test `ensureFontLoaded` cleans up link on resolve

### Task 1.5 — app/utils/__tests__/phrases.test.ts
- [ ] Test `getPhrase('daily')` returns a non-empty string
- [ ] Test `getPhrase('inbox')` returns a non-empty string
- [ ] Test `getPhrase('upcoming')` returns a non-empty string
- [ ] Test `getPhrase('monthly')` returns a non-empty string
- [ ] Test `getPhrase('habits')` returns a non-empty string
- [ ] Test invalid key returns undefined or throws

### Task 1.6 — api/src/config.test.ts (extend existing)
- [ ] Cover remaining env var fallback branches
- [ ] Test `DATABASE_URL` default
- [ ] Test `REDIS_URL` default
- [ ] Test `JWT_SECRET` throws if unset
- [ ] Test `CORS_ORIGIN` default
- [ ] Test `PORT` default
...
- [ ] Call handler with any req
- [ ] Verify `res.status(404).json()` called with `{ error: { code: "NOT_FOUND", message: "..." } }`

### Task 2.2 — api/src/middleware/__tests__/errorHandler.test.ts
- [ ] Create `AppError` with statusCode 400, code "BAD_REQUEST", message "invalid input"
- [ ] Call handler with AppError → verify `res.status(400).json()` with `{ error: { code: "BAD_REQUEST", message: "invalid input" } }`
- [ ] Call handler with AppError that has `details` → verify details in response
- [ ] Call handler with generic `Error("unexpected")` → verify `res.status(500).json()` with requestId, console.error called
- [ ] Verify `next` is never called (error handler is terminal)

### Task 2.3 — api/src/middleware/__tests__/csrf.test.ts
- [ ] `GET` request → `res.cookie()` called with `XSRF-TOKEN` (non-httpOnly, sameSite strict)
- [ ] `HEAD` request → cookie set (safe method)
- [ ] `OPTIONS` request → cookie set (safe method)
- [ ] `POST` with matching `x-xsrf-token` header + cookie → calls `next()`
- [ ] `POST` with mismatched header vs cookie → `res.status(403).json()` with error
- [ ] `POST` with missing header → `res.status(403).json()` with error
- [ ] `PATCH` with matching token → calls `next()`
- [ ] `DELETE` with matching token → calls `next()`

### Task 2.4 — api/src/middleware/__tests__/auth.test.ts
- [ ] Mock `sessionService.validateSession` to return `{ user_id: "u1", id: "s1" }`
- [ ] Mock `buildCookieName` to return `"planner_session"`
- [ ] Mock `shouldTouch` to return true/false as needed
- [ ] Mock `touchSession` to return void
- [ ] Valid session cookie → `req.userId` = "u1", `req.sessionId` = "s1", `next()` called
- [ ] Missing cookie → `res.status(401).json()` with auth error
- [ ] Invalid session (validateSession returns null) → 401
- [ ] Session touched when `shouldTouch` returns true
- [ ] Session not touched when `shouldTouch` returns false
