...
- [x] 1.1 Sources taken from the generator zips (not committed to the repo):
  - PWABuilder `android/launchericon-512x512.png` - transparent RGBA master, alpha bbox `(99, 35, 426, 472)`
  - Progressier's device list + media queries for the launch images
...
- [x] 1.2 A one-off Pillow script (LANCZOS on premultiplied `RGBa`, `optimize=True`, not committed) crops the master to its alpha bbox and writes into `app/public/`:
...
- [x] 4.2 Playwright screenshots to `app/dist/screenshots/` (posted in a PR comment): `login-desktop`, `login-mobile`, `app-desktop`, `sidebar-logo`, `icons-sheet` (tab, Pixel circle, Samsung squircle, iPhone, desktop install, Android launch, in-app logos), `ios-launch-screens`.
