...
| # | Decision | Why |
|---|---|---|
| 1 | **Dedicated `history_entries` table**, not a reuse/extension of `activity_events`. | `activity_events` is an unbounded audit log with partial before-data (e.g. delete only stores `{title}`) and no undone/redone state. History needs full reversible snapshots, a bounded ring buffer, and a `status` column (`active`/`undone`) that `activity_events` was never designed to carry. Keeping them separate avoids conflating "audit trail" with "undo stack" semantics — one is append-only forever, the other is a bounded, mutable stack. |
| 2 | **Per-user, not per-session/per-tab.** History is stored server-side keyed by `user_id` and shared across all of that user's tabs/devices. | The feature request says nothing about per-tab isolation, and "history of my actions" reads naturally as "mine everywhere," matching how `preferences` and `tasks` are already user-scoped, not session-scoped. A per-tab stack would silently lose entries on tab close/reload, which contradicts "app saves N entries" (implies durability). |
...
| 10 | **Chord-based keyboard matcher needs a real extension, not a workaround.** `hooks/shortcuts.ts`'s `KeyEvent`/`Binding` types have zero concept of modifier keys today (`SingleBinding.key` is a bare string like `'q'`). Ctrl+Z, Cmd+Z, Shift+Ctrl+Z, Ctrl+Y cannot be expressed with the current matcher. | This is flagged as required scope, not an incidental detail — see Frontend section below for the proposed minimal extension (`modifiers?: { ctrlOrCmd?: boolean; shift?: boolean }` on `SingleBinding`), designed to be additive and not break existing bare-key/chord bindings. |
...
Design drafted self-directed (no interactive review yet). **Not approved.** Requires user review
of the decisions above — especially #1 (new table vs. reusing `activity_events`), #2 (per-user vs.
per-session scope), and #10 (shortcut matcher extension) — before any implementation work starts.
